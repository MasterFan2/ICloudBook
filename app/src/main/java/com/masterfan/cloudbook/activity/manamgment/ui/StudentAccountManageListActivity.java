...
import com.masterfan.cloudbook.activity.manamgment.entity.StudentAccount;
 ...
 * 学生账户管理列表
 */
@MTFActivityFeature(layout=R.layout.activity_student_account_manage_list, status_bar_color = R.color.colorPrimary)
public class StudentAccountManageListActivity extends MTFBaseActivity implements SwipeRefreshLayout.OnRefreshListener{
    ...
    @Bind(R.id.activity_student_account_manage_list_recyclerView_refreshLayout)
    ...
    //recyclerView data
    private ArrayList<StudentAccount> dataList = new ArrayList<>();

    @Bind(R.id.activity_student_account_manage_list_recyclerView)
    ...
    private void initMettingFrament() {
        for (int i = 0; i < 10; i++) {
            dataList.add(new StudentAccount("2017级1班","张三","张三儿","男","正常"));
        }
...
//        MTFRecyclerViewUtils.setHeaderView(recyclerView, headerView);
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                case 0:
                    for (int i = 0; i < 10; i++) {
                        dataList.add(new StudentAccount("2017级1班","张三","张三"+i,"男","正常"));
                    }
    ...
    public void onRefresh() {
        dataList .clear();
        for (int i = 0; i < 10; i++) {
            dataList.add(new StudentAccount("2017级1班","张三"+i,"张三儿","男","正常"));
        }
        currentSize = dataList.size();
    ...
    public class MyAdapter extends RecyclerView.Adapter<MyAdapter.MTFViewHolder> {
        ...
        public MTFViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new MTFViewHolder(mLayoutInflater.inflate(R.layout.item_student_account_manage, parent, false));
        }

        @Override
        public void onBindViewHolder(MTFViewHolder holder, int position) {
            StudentAccount item = dataList.get(position);
            holder.nameTxt.setText(item.getName());
            holder.theClassTxt.setText(item.getTheClass()+"");
        ...
        public class MTFViewHolder extends RecyclerView.ViewHolder {
            ...
            @Bind(R.id.item_student_account_manage_class_textview)
            TextView theClassTxt;

            @Bind(R.id.item_student_account_manage_name_textview)
            TextView nameTxt;

            @Bind(R.id.item_student_account_manage_nickname_textview)
            TextView nickNameTxt;

            @Bind(R.id.item_student_account_manage_sex_textview)
            TextView sexTxt;

            @Bind(R.id.item_student_account_manage_state_textview)
            TextView stateTxt;
