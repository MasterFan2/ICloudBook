...
public class RecentlyViewedListFragment extends MTFBaseFragment implements SwipeRefreshLayout.OnRefreshListener{
	...
	private void initMettingFrament() {
		for (int i = 1; i < 3; i++) {
			dataList.add( new BookManagement("西游1记","吴承恩",12,11,22,33,44,"2012-12-12"));
		}
...
//        MTFRecyclerViewUtils.setHeaderView(recyclerView, headerView);
	...
	public class MyAdapter extends RecyclerView.Adapter<MyAdapter.MTFViewHolder> {
		...
		public MTFViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			return new MTFViewHolder(mLayoutInflater.inflate(R.layout.item_recently_viewed, parent, false));
		}
		...
		public class MTFViewHolder extends RecyclerView.ViewHolder {
			...
			@Bind(R.id.item_recently_viewed_title_img)
			ImageView titleImg;

			@Bind(R.id.item_recently_viewed_bookname_textview)
			TextView bookNmeTxt;//

			@Bind(R.id.item_recently_viewed_writer_textview)
			TextView writerTxt;//

			@Bind(R.id.item_recently_viewed_has_read_textview)
			TextView hasReadTxt;//

			@Bind(R.id.item_recently_viewed_collect_textview)
			TextView collectTxt;//

			@Bind(R.id.item_recently_viewed_download_textview)
			TextView downloadTxt;//

			@Bind(R.id.item_recently_viewed_read_textview)
			TextView readTxt;

			@Bind(R.id.item_recently_viewed_comment_textview)
			TextView commentTxt;

			@Bind(R.id.item_recently_viewed_time_textview)
			TextView timeTxt;

			@Bind(R.id.item_recently_viewed_continue_read_textview)
			TextView continueRedTxt;

			@Bind(R.id.item_recently_viewed_delete_textview)
			TextView deleteTxt;
