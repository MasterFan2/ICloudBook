...
public class AttentionMineFragment extends MTFBaseFragment implements SwipeRefreshLayout.OnRefreshListener{
	...
	private void initMettingFrament() {
		for (int i = 1; i < 3; i++) {
			dataList.add( new Person("李四","教授",22,33,44,"关注我的哟"));
		}
			...
			if (person != null) {
				...
				holder.attentionTxt.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						Toast.makeText(getActivity(), "关注" + dataList.get(position).getName(), Toast.LENGTH_SHORT).show();
					}
				});
