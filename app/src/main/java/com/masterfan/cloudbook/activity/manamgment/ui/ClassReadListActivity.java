 ...
 * 班级阅读列表
 */
@MTFActivityFeature(layout=R.layout.activity_class_read_list, status_bar_color = R.color.colorPrimary)
public class ClassReadListActivity extends MTFBaseActivity implements SwipeRefreshLayout.OnRefreshListener{
    ...
    private void initMettingFrament() {
        for (int i = 0; i < 10; i++) {
            dataList.add(new ClassRead("张三",4,5,6,7,5));
        }
...
//        MTFRecyclerViewUtils.setHeaderView(recyclerView, headerView);
    ...
    public class MyAdapter extends RecyclerView.Adapter<MyAdapter.MTFViewHolder> {
        ...
        public MTFViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new MTFViewHolder(mLayoutInflater.inflate(R.layout.item_class_read, parent, false));
        }

        @Override
        public void onBindViewHolder(MTFViewHolder holder, int position) {
            ClassRead item = dataList.get(position);
            holder.nameTxt.setText(item.getName());
            holder.readNumTxt.setText(item.getReadNum()+"");
            holder.readTimeTxt.setText(item.getReadTime()+"");

            holder.commentNumTxt.setText(item.getCommentNum()+"");
            holder.colledtNumTxt.setText(item.getCommentNum()+"");
            holder.recommendNumTxt.setText(item.getRecommendNum() + "");
        ...
        public class MTFViewHolder extends RecyclerView.ViewHolder {
            ...
            @Bind(R.id.item_class_read_name_textview)
            TextView nameTxt;

            @Bind(R.id.item_class_read_read_number_textview)
            TextView readNumTxt;

            @Bind(R.id.item_class_read_read_time_textview)
            TextView readTimeTxt;

            @Bind(R.id.item_class_read_comment_textview)
            TextView commentNumTxt;

            @Bind(R.id.item_class_read_collect_textview)
            TextView colledtNumTxt;

            @Bind(R.id.item_class_read_recommend_textview)
            TextView recommendNumTxt;
